...
import { IconCalendar, IconCheck, IconDownload, IconList, IconSettings, IconUsers } from '../components/Icons'
...
export default function SchedulePage() {
  ...
  useEffect(() => {
    if (!month.holidaysConfirmed && month.holidays.length === 0) {
      ...
      if (proposed.length > 0) setMonth(monthKey, (m) => ({ ...m, holidays: proposed }))
    }
  ...
  function setRestTarget(empId, n) {
    setMonth(monthKey, (m) => ({
      ...
      fulltimeRest: { ...m.fulltimeRest, [empId]: autoFulltimeRest(data, { ...m, fulltimeRestTarget: { ...m.fulltimeRestTarget, [empId]: n } }, monthKey, empId) },
    }))
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="font-display text-xl font-bold text-main">排班 · {monthLabel(monthKey)}</h1>
        <div className="flex flex-wrap gap-2">
          <button className="btn-ghost" onClick={() => setModal('holiday')}>
            <IconCalendar width={15} height={15} /> 國定假日 ({month.holidays.length})
          </button>
          ...
          <button className="btn-ghost" onClick={() => setModal('export')} disabled={!hasSchedule}>
      ...
      {!month.holidaysConfirmed && (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-warning bg-warning-bg px-4 py-3 text-sm text-main">
          ...
          <button className="btn-primary py-1.5" onClick={() => setModal('holiday')}>前往確認</button>
        </div>
      )}
      ...
      <div className="card flex flex-wrap items-center gap-x-5 gap-y-3 p-3">
        ...
        {fulltimers.map((e) => (
          <div key={e.id} className="flex items-center gap-1.5">
            ...
            <input
              ...
              onChange={(ev) => setRestTarget(e.id, Math.max(0, Number(ev.target.value) || 0))}
            />
        ...
        <div className="ml-auto flex gap-2">
          {hasSchedule && <button className="btn-ghost" onClick={clearSchedule}>清空</button>}
          <button className="btn-primary" onClick={() => autoSchedule(monthKey)}>
            <IconCheck width={16} height={16} /> {hasSchedule ? '重新自動排班' : '自動排班'}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        <div className="min-w-0 flex-1">
          {hasSchedule ? (
            <ScheduleGrid onEditCell={(date, shiftKey, defStart, defEnd) => setEditing({ date, shiftKey, defStart, defEnd })} />
          ) : (
      ...
      <SpecialDaysModal open={modal === 'special'} onClose={() => setModal(null)} />
      <AvailabilityModal open={modal === 'avail'} onClose={() => setModal(null)} />

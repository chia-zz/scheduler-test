...
import { continuousWarnings, getDaySlots, isNationalHoliday } from '../scheduler'

export default function ScheduleGrid({ onEditCell }) {
  const { data, monthKey, currentMonth } = useApp()
  const month = currentMonth
  const empById = useMemo(() => Object.fromEntries(data.employees.map((e) => [e.id, e])), [data.employees])
  const warnings = useMemo(() => continuousWarnings(monthKey, month), [monthKey, month])
    ...
    return (
      <div className="min-h-[118px] rounded-xl border border-line bg-surface p-2 sm:min-h-[130px]">
        <div className="mb-1 flex items-baseline justify-between">
          <span className={`font-display text-sm font-semibold ${holiday || w >= 5 ? 'text-error' : 'text-main'}`}>
            {dayOfMonth(date)}
          </span>
          {holiday && <span className="truncate text-[9px] text-error">{holiday.name}</span>}
        </div>
        {closed ? (
          <div className="rounded-md bg-error-bg px-1.5 py-2 text-center text-[11px] text-error">公休</div>
        ) : (
          <div className="space-y-1">
            ...
            {resting.map((e) => (
              <div key={e.id} className="flex items-center gap-1 rounded-md bg-bg px-1.5 py-0.5 text-[10px] text-sub">
      ...
      <div className="min-w-[760px]">
        <div className="mb-1 grid grid-cols-7 gap-1.5">
          {WEEKDAY_SHORT.map((wd, i) => (
            <div key={wd} className={`text-center text-xs font-semibold ${i >= 5 ? 'text-error' : 'text-sub'}`}>
              星期{wd}
            </div>
          ))}
        </div>
        <div className="space-y-1.5">
          {weeks.map((week, wi) => (
            <div key={wi} className="grid grid-cols-7 gap-1.5">
              {week.map((d, di) => (d ? <DayCell key={d} date={d} /> : <div key={di} />))}
            </div>
          ))}
        </div>
      </div>

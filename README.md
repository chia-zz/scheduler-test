...
- 不使用任何後端、資料庫或第三方登入服務。
- 不使用 React Router；以分頁（tab）狀態切換頁面，方便 GitHub Pages 部署。
...
- **匯出備份**：設定頁 →「匯出 JSON 備份」，檔名自動帶月份（如 `班表備份_2026-04.json`）。
- **匯入備份**：設定頁 →「匯入 JSON 備份」，會覆蓋目前全部資料。
- **跨裝置同步**：用匯出 / 匯入達成——在 A 電腦匯出，到 B 電腦匯入即可。
- **重設**：設定頁 →「重設為預設值」會清空所有資料並還原預設員工（密碼也會清除），重設前建議先匯出備份。
...
| 桌機最大寬度                        | `src/App.jsx`、`src/components/Header.jsx`        | `max-w-[1400px]`                                        |

### 每年更新國定假日

1. 最簡單：在排班頁「國定假日」彈窗按「**從網路更新**」即可抓取該年度（資料來源為 TaiwanCalendar）。
